...
int busca_binaria(TipoAluno vet[], int num, int chave)
{
  ...
  cout << " Foram realizadas " << cont << " compara��es\n";
  return (pos);
}
...
int main()
{
  ...
  {
    n = 0;
    fscanf(arq, "%s%s%s", rmS, aluno[n].nome, mediaS); //apresenta o cabe�alho da tabela
    cout << "RM \t Nome \t\tM�dia \n";
    ...
    if (n == -1)
      cout << "Registro n�o encontrado \n";
    else
      cout << "\n nome do aluno: " << aluno[n].nome << " tem m�dia: " << aluno[n].media << endl;
  }
  return 0;
}
